...
- **📊 持仓管理**：实时追踪基金持仓，支持编辑持仓信息
- **📈 数据可视化**：精美的图表展示基金估值曲线和涨跌趋势
- **💼 分组管理**：灵活的自选基金分组，支持自定义分类
- **📱 响应式设计**：完美适配桌面端和移动端
- **🗓️ 交易日判断**：智能识别非交易日（周末/节假日），自动隐藏预估数据
...
- **持仓收益**：使用数据库中的 `holding_profit` 字段，手动输入或编辑时记录
- **预估数据显示规则**：
  - 非交易日（周末/节假日）：隐藏预估收益和预估涨幅
  - 交易日 0:00-9:30：隐藏（尚未开盘）
  - 交易日 9:30-24:00：显示（若估值日期为当天）
- **实际收益显示规则**：
  - 净值日期是今天：显示（今天净值已发布）
