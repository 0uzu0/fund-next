...
基于前端页面的 Android 应用，**无顶部导航**，仅底部 Tab：**持仓基金**、**贵金属行情**、**行业板块**、**用户管理**。内容通过 WebView 加载前端对应页面。

- **与前端独立**：功能与样式由本端控制，不依赖前端“App 模式”或特殊 class；通过注入样式与脚本在 WebView 内移除顶栏/侧栏、预留底部栏与弹窗适配。
